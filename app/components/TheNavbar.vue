...
<template>
  <header class="sticky top-0 z-50 border-b border-hairline-soft bg-white/80 backdrop-blur">
    <div :class="['mx-auto flex h-16 items-center justify-between px-6', width]">
      <div class="flex items-center gap-8">
        <HovlyLink />

        <nav v-if="user" class="hidden md:flex items-center gap-1 text-sm font-medium text-steel">
          <NuxtLink
            to="/dashboard"
            class="rounded-full px-3.5 py-1.5 transition"
            :class="route.path === '/dashboard' ? 'bg-ink text-white' : 'hover:bg-surface'"
          >
            Mes biens
          </NuxtLink>
          <NuxtLink
            ...
            :class="route.path === '/alertes' ? 'bg-ink text-white' : 'hover:bg-surface'"
          >
            Alertes
            <span
              v-if="nonVues > 0"
        ...
        <div v-else-if="showLinks" class="hidden md:flex items-center gap-8 text-sm text-slate">
          <a
            v-for="a in anchors"
            :key="a.href"
            :href="a.href"
