...
const localises = computed(() => props.biens.filter((b) => b.lat != null && b.lon != null))
const sansPosition = computed(() => props.biens.length - localises.value.length)
...
function popup(b: Bien): string {
  const prix = b.prix ? `${eur(Math.round(b.prix / 100))} €/mois` : 'Prix inconnu'
  const m2 = b.surface ? ` · ${eur(Math.round(b.prix / 100 / b.surface))} €/m²` : ''
  const approx =
    b.geo_precision === 'ville' ? '<div class="mt-1 text-stone">Position approximative</div>' : ''
  return `
    <div class="text-sm">
      <a href="/bien/${b.id}" class="font-semibold text-ink hover:underline">${echapper(b.titre ?? 'Sans titre')}</a>
      <div class="mt-1 text-slate">${prix}${m2}</div>
      <div class="text-stone">${echapper([b.ville, b.code_postal].filter(Boolean).join(' '))}</div>
      ${approx}
    </div>`
}
...
async function dessiner() {
  ...
  if (localises.value.length === 1) {
    ...
    carte.fitBounds(
      L.latLngBounds(localises.value.map((b) => [b.lat!, b.lon!] as [number, number])),
...
<template>
  <div class="flex min-h-0 flex-col">
    ...
    <p v-if="sansPosition > 0" class="mt-2 text-xs text-stone">
      {{ sansPosition }} bien{{ sansPosition > 1 ? 's' : '' }} sans localisation — adresse trop
      imprécise dans l’annonce.
    </p>
  </div>
</template>
